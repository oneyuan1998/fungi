...
class TransUnet():
    ...
    def build_model(self):
        ...
        model = tfk.models.Model(inputs=x, outputs=y, name=self.name)
        model.summary()
        return model
...
def build_model(config, trainable=True, pretrained_weights=None):
    # Tranformer Encoder
    assert config.image_size % config.patch_size == 0, "image_size must be a multiple of patch_size"
    x = tf.keras.layers.Input(shape=(config.image_size, config.image_size, 3))

    # Embedding
    if config.hybrid:
        grid_size = config.grid
        config.patch_size = config.image_size // 16 // grid_size[0]
        if config.patch_size == 0:
            config.patch_size = 1
        
        if trainable:
            config.resnet50v2 = ResNetV2(
                block_units=config.resnet.n_layers)
            y, features = config.resnet50v2(x)
        else:
            resnet50v2, features = config.resnet_embeddings(x)
            y = resnet50v2.get_layer("conv4_block6_preact_relu").output
            x = resnet50v2.input
    else:
        y = x
    ...
    y = tfkl.Reshape(
        (y.shape[1] * y.shape[2], config.hidden_size))(y)
    y = encoder_layers.AddPositionEmbs(
    ...
    y = tfkl.Reshape(
        target_shape=[n_patch_sqrt, n_patch_sqrt, config.hidden_size])(y)

    # Decoder CUP
    if "decoder_channels" in config:
        y = decoder_layers.DecoderCup(
            decoder_channels=config.decoder_channels, n_skip=config.n_skip)(y, features)

    # Segmentation Head
    y = decoder_layers.SegmentationHead(
        filters=3, kernel_size=config.kernel_size, upsampling_factor=config.upsampling_factor)(y)
    
    y = Activation('softmax')(y)

    model = tfk.models.Model(inputs=x, outputs=y, name=config.name)
...
# a = np.arange(0,224*224*3,1)
# a = a.astype(np.float32)
# b = tf.convert_to_tensor(a)
# b = tf.reshape(b,(1,224,224,3))
# y = model(b)
# model.summary()